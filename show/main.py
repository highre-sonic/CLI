...
def run_show_ntp_command(command, display_cmd=False, return_cmd=False):
    if display_cmd:
        click.echo(click.style("Command: ", fg='cyan') + click.style(command, fg='green'))
...
def address ():
    ...
    for key in natsorted(list(mgmt_ip_data.keys())):
        ...
        if mgmt_ip_data[key]['gwaddr'] != "NULL" :
...
def interfaces(namespace, display):
    """Show interface IP addresses""" #fix - help of IP Interfaces#
...
@multi_asic_util.multi_asic_click_options
def interfaces(namespace, display):
    """Show interface IPv6 addresses""" #fix - help of IPv6 Interfaces#
...
def ntp(ctx, verbose):
    ...
    run_show_ntp_command(ntpstat_cmd, display_cmd=verbose)
    run_show_ntp_command(ntpcmd, display_cmd=verbose)
...
#######################################
#Function Name : https, status
#Purpose       : show https status (whether check mode is enable or disable)
#Created By    : INDIA_SONIC
#Parameter     : db - config_db , table - HTTPS
#output        : mode :enable / mode :disable
#######################################
#show https status start#
@cli.group(name='https', cls=clicommon.AliasedGroup)
def https():
    """Show HTTPS information"""
    pass

@https.command()
@click.option('--verbose', is_flag=True, help="Enable verbose output")
@click.option('--port_no', is_flag=True, help="Port Number")
def status(verbose, port_no):
    """Show HTTPS status """

    https_port = COMMON.get_https_port()
    https_mode = COMMON.get_https_mode()
    web_session_idle_timeout = COMMON.get_web_session_idle_timeout()
    
    click.echo("HTTPS Mode : {0}".format(https_mode))
    click.echo("HTTPS Port Number : {0}".format(https_port))
    click.echo("Web Session Idle Timeout : {0} minutes".format(web_session_idle_timeout))
#show https status - end# 
