...
import web_operations as COMMON
...
# Prasanna: Group system is added to config, right now two commands as below
# are available in it such as
# config system reset-to-factory
# and
# config system setup-all-ports-to-l2 --vlan <vlannumber>
#
@config.group(cls=clicommon.AbbreviationGroup)
@click.pass_context
def system(ctx):
    if os.geteuid() != 0:
        ctx.fail("Root privileges are required for this operation")

@system.command()
@click.pass_context
@click.option('-y', '--yes', is_flag=True, callback=_abort_if_false,
                expose_value=False, prompt='This command will reset settings to factory defaults.After resetting to factory defaults, all configs willl be lost and switch will be reloaded immediately.Do you really want to execute this command and reload the switch?')
def reset_to_factory(ctx):
    """ Reset to factory default """
    if os.path.isfile('/usr/local/bin/reset-to-factory-default') is False:
        ctx.fail("rest-to-factory-default feature unavailable in this image version")

    cmd = "touch /tmp/sonic_cli_factory_reset"
    clicommon.run_command(cmd, display_cmd=False)

    command = "/usr/local/bin/reset-to-factory-default"
    clicommon.run_command(command, display_cmd=False)

# This command is a conversion of the script set_all_ports_to_l2.py,
# to a sonic compatible command. The user validations of the scripts
# are sckipped, since these are done as click infrastructure.
@system.group(cls=clicommon.AbbreviationGroup, name='set-all-ports-to-l2')
@click.pass_context
def set_all_ports_to_l2(ctx):
    """ Set all ports to l2. """
    pass

commands_one =  "show ip int"
output_commands_one = os.popen(commands_one).read()
output_commands_one = str(output_commands_one)
pattern_int = re.findall("Ethernet[0-9]+",output_commands_one)
pattern_ip = re.findall("(?:[0-9]{1,3}\.){3}[0-9]{1,3}\/[0-9]+",output_commands_one)
pattern_ip = pattern_ip[:len(pattern_ip)-4]


# Given a list, write a Python program to convert the given list to dictionary such that all the odd elements have the key,
# and even number elements have the value
...
@set_all_ports_to_l2.command('vlan')
@click.pass_context
@click.argument('vlan', metavar='<vlan>', required=True, type=click.IntRange(2,4094))
@click.option('-y', '--yes', is_flag=True, callback=_abort_if_false,
                expose_value=False, prompt='This command will make all the ports part of the given vlan. \nPlease take a backup of the current config before proceeding.\nDo you want to execute this command now?')
def vlan(ctx, vlan):
    """ Set the switch to l2 mode, by putting all ports to the given vlan.
    """
    command1 = 'sudo config vlan add ' + str(vlan)
    clicommon.run_command(command1,display_cmd=True)
    final_list = []
    for i in range(len(pattern_int)):
        final_list.append(pattern_int[i])
        final_list.append(pattern_ip[i])

    final_list = Convert(final_list)

    for key in final_list:
        ip_command = 'sudo config interface ip remove ' + key + " " + final_list[key] + '\n'
        vlan_command = 'sudo config vlan member add -u ' + str(vlan) + ' ' + key + '\n'
        clicommon.run_command(ip_command,display_cmd=True)
        clicommon.run_command(vlan_command,display_cmd=True)
...
#        command2 = ('sudo config interface ip remove Ethernet' + str(interface) + " " + '10.0.0.' + str(interface*2) + '/31' + '\n')
#        clicommon.run_command(command2,display_cmd=True)
#        command3 =  ("sudo config vlan member add -u " + str(vlan) +" Ethernet" + str(interface) + '\n')
#        clicommon.run_command(command3,display_cmd=True)
...
def add_portchannel(ctx, portchannel_name, min_links, fallback):
    """Add port channel, name should have prefix 'PortChannel' and suffix '<0-9999>'""" #fix - add portchannel help#
...
def remove_portchannel(ctx, portchannel_name):
    """Remove port channel, name should have prefix 'PortChannel' and suffix '<0-9999>'""" #fix - delete portchannel help#
    if is_portchannel_name_valid(portchannel_name) != True:
        ctx.fail("{} is invalid!, name should have prefix '{}' and suffix '{}'"
                 .format(portchannel_name, CFG_PORTCHANNEL_PREFIX, CFG_PORTCHANNEL_NO))
...
def add_portchannel_member(ctx, portchannel_name, port_name):
    ...
    for k,v in db.get_table('PORTCHANNEL_MEMBER'):
        if k == portchannel_name:
            #Prasanna Verify that the number of member ports in a port channel does not exceed 8
            #Issue 143524
            member_count += 1
            if member_count == 8:
                ctx.fail("Maximum 8 member ports are allowed in portchannel {}".format(portchannel_name))
...
def add_snmp_agent_address(ctx, agentip, port, vrf):
    ...
    if not clicommon.is_ipaddress(agentip):
        ...
        return False
    #fix - snmp ip address check - start#
    try:
        ip_address = ipaddress.ip_interface(agentip)
    except ValueError as err:
        ctx.fail("IP address is not valid: {}".format(err))
    #fix - snmp ip address check - end#
    config_db = ctx.obj['db']
    if not vrf:
        entry = config_db.get_entry('MGMT_VRF_CONFIG', "vrf_global")
        if entry and entry['mgmtVrfEnabled'] == 'true' :
    ...
    for intf in netifaces.interfaces():
        ...
        click.echo("IP address is not available") #fix - spelling mistake#
        return
...
def modify_snmptrap_server(ctx, ver, serverip, port, vrf, comm):
    ...
    #fix - snmp ip address check - start#
    try:
        ip_address = ipaddress.ip_interface(serverip)
    except ValueError as err:
        ctx.fail("IP address is not valid: {}".format(err))
    #fix - snmp ip address check - end#
...
def autoneg(ctx, interface_name, mode, verbose):
    """Set interface auto negotiation mode [enabled,disabled]""" #fix - config autoneg help#
    # Get the config_db connector
    config_db = ctx.obj['config_db']
...
def interface_type(ctx, interface_name, interface_type_value, verbose):
    try:
        raise ValueError("This command is not supported in this platform!")
    except ValueError as e:
        print(e)
        exit()
    """Set interface type [CR4 | CR2 | XGMII | CAUI | CAUI4 | XFI | LR | KR | SR4 | SR2 | none | KR4 | XAUI | LR4 | SFI | GMII | KR2 | XLAUI | SR | CR]""" #fix - type enumeration#
    # Get the config_db connector
    config_db = ctx.obj['config_db']
...
def advertised_types(ctx, interface_name, interface_type_list, verbose):
    try:
        ...
        print(e)
        exit()
    """Set interface advertised types [CR4 | CR2 | XGMII | CAUI | CAUI4 | XFI | LR | KR | SR4 | SR2 | none | KR4 | XAUI | LR4 | SFI | GMII | KR2 | XLAUI | SR | CR]""" #fix - advertised type enumeration#"""
    # Get the config_db connector
    config_db = ctx.obj['config_db']
...
@click.pass_context
@click.argument('interface_name', metavar='<interface_name>', required=True)
#Issue 147824, Setting the mtu range between 1500-9100, default is 9100 defined in portsorch.cpp
@click.argument('interface_mtu', metavar='<interface_mtu>', type=click.IntRange(1500,9100), required=True)
@click.option('-v', '--verbose', is_flag=True, help="Enable verbose output")
def mtu(ctx, interface_name, interface_mtu, verbose):
    """Set interface mtu (1500-9100)""" #fix - mtu range#
    #Issue 147824, not allowed to configure odd mtu to SAI.
    if (int(interface_mtu) % 2):
        ctx.fail("Configured MTU value must be an even number")
...
@click.pass_context
@click.argument('interface_name', metavar='<interface_name>', required=True)
@click.argument('interface_fec', metavar='<interface_fec>', type=click.Choice(["rs", "fc", "none"]), required=True) #fix - fec enumeration#
@click.option('-v', '--verbose', is_flag=True, help="Enable verbose output")
def fec(ctx, interface_name, interface_fec, verbose):
    """Set interface fec [rs | fc | none]""" #fix - fec enumeration#
    # Get the config_db connector
    config_db = ctx.obj['config_db']
...
def add(ctx, interface_name, ip_addr, gw):
    ...
    if interface_name == 'eth0':
        ...
        if not gw:
            config_db.set_entry("MGMT_INTERFACE", (interface_name, str(ip_address)), {"gwaddr": "NULL"})
        else:
...
def add_loopback(ctx, loopback_name):
    """Add Loopback, name should have prefix 'Loopback' and suffix '<0-999>'""" #fix - add loopback help#
    config_db = ctx.obj['db']
...
def del_loopback(ctx, loopback_name):
    """Remove Loopback, name should have prefix 'Loopback' and suffix '<0-999>'""" #fix - delete loopback help#
    config_db = ctx.obj['db']
    if is_loopback_name_valid(loopback_name) is False:
        ctx.fail("{} is invalid, name should have prefix '{}' and suffix '{}' "
                .format(loopback_name, CFG_LOOPBACK_PREFIX, CFG_LOOPBACK_NO))
...
def set_sdk_diag_shell(ctx, en):
    ...
    subprocess.Popen(command, shell=True)
##################################################################################
#Function Name : https, mode
#Purpose       : To configure the HTTPS
#Created By    : INDIA_SONIC
#Parameters    : mode, enable, disable( DB, Table:config_db , WEB_TABLE)
#output        : It configures HTTPS mode changed into enable/ disable
#################################################################################
#sudo config https mode - start#
@config.group()
@click.pass_context
def https(mode):
    """HTTPS Configuration"""
...
@https.command('mode')
@click.argument('mode',metavar='<enable|disable>', required=False)
def mode(mode):
    
    """Configuring the HTTPS mode choose from enable, disable"""
   
    if mode == COMMON.HTTPS_ENABLE or mode == COMMON.HTTPS_DISABLE:
        
        COMMON.CONFIG_DB = ConfigDBConnector()
        COMMON.CONFIG_DB.connect()

        COMMON.CONFIG_DB.mod_entry(COMMON.WEB_TABLE, COMMON.WEB_TABLE_KEY, {COMMON.HTTPS_MODE:mode})
               
        click.echo("HTTPS Mode changed into {}".format(mode))
        process_status = COMMON.configure_lighttpd_mode(mode)
         
    else:
        raise click.BadParameter("choose from enable , disable")
#sudo config https mode - end#

##############################################################################
#Function Name : port
#Purpose       : To configure the port number
#Created By    : INDIA_SONIC
#Parameters    : port no,( DB, TABLE : config db, WEB_TABLE)
#output        : it return the port number
##############################################################################
#sudo config https port - start#
@https.command('port')
@click.argument('port_no', metavar='https_port',required=False,type=click.IntRange(0, 65535))
def port(port_no):
    """ Port Number Configuration ranges between <0 - 65535>"""
    COMMON.CONFIG_DB = ConfigDBConnector()
    COMMON.CONFIG_DB.connect()
       
    port_status = COMMON.check_lighttpd_status(port_no)
    if (port_status == COMMON.HTTPS_PORT_ALREADY_IN_USE):
        click.echo("Port {} is already in use".format(port_no))
    else:
        COMMON.CONFIG_DB.mod_entry(COMMON.WEB_TABLE, COMMON.WEB_TABLE_KEY, {COMMON.HTTPS_PORT:port_no})
        click.echo("HTTPS Port No {}".format(port_no))
    
        COMMON.modify_lighttpd_conf_file(port_no)
        COMMON.CONFIG_DB.mod_entry(COMMON.WEB_TABLE, COMMON.WEB_TABLE_KEY, {COMMON.HTTPS_MODE:COMMON.HTTPS_ENABLE})
        process_status = COMMON.configure_lighttpd_mode(COMMON.HTTPS_ENABLE) 
        
#sudo config https port - end#

##############################################################################
#Function Name : session_timeout
#Purpose       : To configure the Port session timing
#Created By    : INDIA_SONIC
#parameters    : session_timeout, (DB, TABLE : config db, WEB_TABLE)
#output        : It return the session_timeout
##############################################################################
@https.command('web_session_idle_timeout')
@click.argument('session_timeout', metavar = '<session_timeout>' , type=click.IntRange(5, 120),required = False)
def session_timeout(session_timeout):
    """ Session Timeout configuration <5 - 120> minutes format"""

    COMMON.CONFIG_DB = ConfigDBConnector()
    COMMON.CONFIG_DB.connect()

    COMMON.CONFIG_DB.mod_entry(COMMON.WEB_TABLE, COMMON.WEB_TABLE_KEY,{COMMON.WEB_SESSION_IDLE_TIMEOUT : session_timeout})

    click.echo("HTTPS web session time {}".format(session_timeout))
#sudo config session timeout- end#
